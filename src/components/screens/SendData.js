                    ...
                    JSON.parse(list).map(item=>{
                        if(item.saveType === "network" && selector.editingService !== item.projectId){
                            if (!!item.startLatitude){
                                ...
                                RNFetchBlob.fs.readFile(`${dirs.DownloadDir}/${item.projectId}/2.png`, 'base64').then(async data=>{
                                    if(!!data){
                                        BILL_IMAGE =await data;
                                    }
                                });
                                RNFetchBlob.fs.readFile(`${dirs.DownloadDir}/${item.projectId}/3.png`, 'base64').then(async data=>{
                                    if(!!data){
                                        IMAGE = await data;
                                    }
                                });
                                sendServiceData(
                                    ...
                                    if (data.errorCode === 0){
                                        ...
                                        AsyncStorage.getItem("savedServicesList").then(list=>{
                                            let tempList = JSON.parse(list).filter(service=>service.projectId !== item.projectId);
                                            AsyncStorage.setItem("savedServicesList", JSON.stringify(tempList));
                                            RNFetchBlob.fs.unlink(`${dirs.DownloadDir}/${item.projectId}`);
                                        });
